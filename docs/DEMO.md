...
## Storyboard (target 1:45, hard limit < 2:00)
...
| 2 | record | 7 | Record screen, timer running | **Record in class** — It keeps recording with the screen off (a microphone foreground service). |
| 3 | import | 12 | Downloads picker → "Signals and Systems" → Build | **...or use any recording** — Pick the file and name the class. |
| 4 | processing | 8 | Named steps + real seconds counter (not sped up) | **One Gemini call listens** — strict JSON schema; the app checks every rule and retries once with the reasons. |
| 5 | card | 14 | 2 exam hints, "What they said", play from 00:38, key ideas, to-dos | **Only what the lecturer said** — exam hints with their exact words, key ideas, homework with the deadline as spoken. |
| 6 | nextday | 20 | Clock +1 day → Today's loop → quiz 4/5 → "Day 1 done", next review day 3 | **The loop: day 1, 3, 7** — clock moved forward for this demo. |
| 7 | paywall | 24 | 3rd lecture → paywall 2/2 → Semester Pass → Test Store sheet → card is built | **2 free lectures a week** — limit from offering metadata; `pro` unlocks; the waiting recording is built straight away. |
| 8 | account | 8 | Account → Customer Center | **Manage it in the app** — Customer Center for cancel and restore. |
| 9 | end | 6 | End card | Open source (MIT) · github.com/Ryugi62/lectureloop · built by a student whose desktop prototype has turned 27 of their own lectures into review pages this semester. |
...
# 3. record every scene and compose the video (needs GEMINI_API_KEY and REVENUECAT_TEST_STORE_KEY in local.properties)
